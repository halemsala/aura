...
def _v_list_files(args) -> dict:
    return {"path": resolve_allowed((args or {}).get("path") or "Desktop", mode="read")}
...
def _v_read(args) -> dict:
    p = resolve_allowed((args or {}).get("path"), mode="read")
    if not p.is_file():
...
register(ToolSpec("write_text_file", write_text_file, _v_write, risk="high", mutating=True, sensitive=False,
                  summary="Escreve ficheiro allowlisted (backup prévio; exige AUTORIZO; mostra o path)",
                  rollback=_rollback_write))


def _v_delete(args) -> dict:
    p = resolve_allowed((args or {}).get("path"), mode="mutate")
    if p.is_dir():
        raise ValidationError("não apago pastas — indica o ficheiro exacto")
    if not p.exists():
        raise ValidationError(f"ficheiro inexistente: {p}")
    return {"path": p}


def delete_file(args, ctx) -> dict:
    p = _v_delete(args)["path"]
    if ctx.dry():
        return {"dry_run": True, "would_delete": [str(p)],
                "nota": "NADA foi apagado. Responde AUTORIZO para apagar este ficheiro (fica backup)."}
    trash = paths.BACKUPS_DIR / "deleted" / time.strftime("%Y%m%d-%H%M%S")
    trash.mkdir(parents=True, exist_ok=True)
    dest = trash / p.name
    shutil.copy2(p, dest)
    p.unlink()
    return {"deleted": True, "path": str(p), "backup": str(dest),
            "nota": "ficheiro movido para backup; não foi destruído sem cópia"}


def _rollback_delete(args, result, ctx) -> bool:
    b = (result or {}).get("backup")
    p = (result or {}).get("path")
    if b and p and Path(b).exists():
        shutil.copy2(b, p)
        return True
    return False


register(ToolSpec("delete_file", delete_file, _v_delete, risk="high", mutating=True, sensitive=False,
                  summary="Apaga UM ficheiro após AUTORIZO. Mostra o path. Backup obrigatório. Sem pastas.",
                  rollback=_rollback_delete))


def _v_preview(args) -> dict:
    p = resolve_allowed((args or {}).get("path") or "Desktop", mode="read")
    return {"path": p}


def preview_change(args, ctx) -> dict:
    p = _v_preview(args)["path"]
    exists = p.exists()
    return {"path": str(p), "exists": exists, "dir": p.is_dir() if exists else False,
            "size": (p.stat().st_size if exists and p.is_file() else None),
            "nota": "pré-visualização — nenhuma alteração feita"}


register(ToolSpec("preview_change", preview_change, _v_preview, risk="low", mutating=False,
                  summary="Mostra o ficheiro que seria modificado, sem alterar nada"))
