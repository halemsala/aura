...
MUTATING_ACTIONS = {
    ...
    "iniciar stack e abrir aura": "Iniciar Engine, Bridge e Voice; validar saúde; depois abrir a interface AURA.",
}
...
class AuraServiceManager:
    ...
    def start(self,name):
        x=self.specs[name]
        if self._health(name): return {"ok":True,"service":name,"status":"already_online","port":x['port']}
        ...
        while time.time()<end:
            if self._health(name): return {"ok":True,"service":name,"status":"started","pid":proc.pid,"port":x['port'],"log":str(logp)}
...
class Halem:
    ...
    def intent(self, raw: str):
        """
        Roteador determinístico. Confirmações e comandos operacionais são
        resolvidos ANTES do modelo para impedir respostas inventadas pelo LLM.
        """
        text = normalize(raw)
        ...
        # 1) CONFIRMAÇÃO TEM PRIORIDADE ABSOLUTA.
        if text in {"cancelar", "cancela", "nao", "não", "pare", "parar"}:
            return "cancel", None

        if text.startswith("confirmar "):
            return "confirm", text.removeprefix("confirmar ").strip()

        if self.pending_action and text in {
            "sim", "s", "confirmo", "confirmado", "confirmada",
            "pode", "pode sim", "prossiga", "continue", "continuar",
            "ok", "okay", "certo", "isso", "execute", "executar"
        }:
            return "confirm", self.pending_action

        # 2) INTENÇÕES COMPOSTAS: iniciar tudo E depois abrir AURA.
        start_words = ("inicie", "iniciar", "ligue", "ligar", "ative", "ativar", "execute", "executar", "rode", "rodar")
        service_words = (
            "todos os servicos", "todos os serviços", "todos servicos",
            "todos serviços", "o sistema", "sistema aura", "a aura",
            "tudo", "stack"
        )
        open_words = ("abra", "abrir", "abertura", "mostrar", "exibir")
        aura_words = ("aura", "desktop", "interface", "matriz")

        wants_start = any(w in text for w in start_words) and any(w in text for w in service_words)
        wants_open = any(w in text for w in open_words) and any(w in text for w in aura_words)

        if wants_start and wants_open:
            return "action", "iniciar stack e abrir aura"

        # 3) COMANDOS OPERACIONAIS INDIVIDUAIS.
        if any(phrase in text for phrase in (
            "inicie todos os serviços", "inicie todos os servicos",
            "iniciar todos os serviços", "iniciar todos os servicos",
            "ligue todos os serviços", "ligue todos os servicos",
            "ative todos os serviços", "ative todos os servicos",
            "ligue tudo", "ative tudo", "inicie tudo",
            "iniciar sistema aura", "iniciar o sistema aura",
            "inicie o sistema", "inicie o aura", "iniciar o aura",
            "ligue o aura", "ligar o aura", "inicie aura"
        )):
            return "action", "iniciar stack"

        if any(phrase in text for phrase in (
            "abra aura", "abra o aura", "abrir aura", "abrir o aura",
            "abra o desktop", "abrir o desktop", "abra a interface",
            "abrir a interface", "abra a matriz", "abrir a matriz",
            "quero operar"
        )):
            return "action", "abrir aura"

        # Correções de grafia e comandos curtos.
        aliases = {
            "brigde": "bridge",
            "bridge": "bridge",
            "voz": "voice",
        ...
        for token, service in aliases.items():
            if token in text:
                if any(w in text for w in ("reinicie", "reiniciar", "restart")):
                    return "action", f"reiniciar {service}"
                if any(w in text for w in ("inicie", "iniciar", "ligue", "ligar", "ative", "ativar")):
                    return "action", f"iniciar {service}"

        # 4) COMANDOS DE STATUS/AUDITORIA.
        if any(phrase in text for phrase in ("auditar aura", "faça auditoria", "faca auditoria", "auditoria", "audite o sistema")):
            return "action", "auditar aura"
        if text in {"status", "estado", "diagnostico", "diagnóstico", "diagnostico completo", "diagnóstico completo", "esta pronto", "está pronto"}:
            return "status", None
        ...
        if any(phrase in text for phrase in ("reinicie a aura", "reiniciar a aura", "reinicie tudo", "reiniciar tudo", "reiniciar o sistema completo")):
    ...
    async def execute(self, action: str):
        ...
        if action == "iniciar stack e abrir aura":
            result = await asyncio.to_thread(self.services.start_stack)
            audit("stack_start_then_open", result=result)
            if not result.get("ok"):
                failed = result.get("failed")
                detail = result["results"][-1]
                return f"🔴 Serviços não ficaram totalmente online. Falha: {failed}.\nStatus: {detail.get('status') or detail.get('error')}\n📄 Log: {detail.get('log','não disponível')}"
            candidates = [AURA_ROOT/"AURA_DESKTOP.exe", AURA_ROOT/"desktop"/"AURA_DESKTOP.exe", AURA_ROOT/"START_AURA.bat"]
            target = next((p for p in candidates if p.exists()), None)
            lines = ["🚀 AURA STACK ONLINE"]
            for r in result["results"]:
                lines.append(f"🟢 {r['service'].title()} · porta {r['port']} · {r['status']}")
            if target:
                subprocess.Popen([str(target)], cwd=str(target.parent), shell=target.suffix.lower()==".bat")
                lines.append(f"🖥️ Abrindo AURA: {target.name}")
            else:
                lines.append("🟡 Serviços online, mas Desktop AURA não encontrado nos entrypoints configurados.")
            return "\n".join(lines)
