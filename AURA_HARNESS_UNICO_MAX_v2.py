...
class AuraServiceManager:
    """Supervisor determinístico dos serviços AURA. Não depende do LLM."""
    def __init__(self, root: Path):
        self.root = root.resolve()
        self.runtime = CONTROL_ROOT / "runtime"
        self.runtime.mkdir(parents=True, exist_ok=True)
        # Usa o Python do ambiente do projeto quando existir.
        vpy = self.root / "engine" / "venv" / "Scripts" / "python.exe"
        py = str(vpy) if vpy.exists() else sys.executable
        self.specs = {
            "engine": {"port":8765, "health":"/api/health", "cmd":[py, str(self.root/"engine"/"server.py")], "cwd":self.root/"engine", "timeout":45},
            "bridge": {"port":8080, "health":"/health", "cmd":[py, str(self.root/"bridge"/"server.py")], "cwd":self.root/"bridge", "timeout":45},
            "voice": {"port":8099, "health":"/api/voice/health", "cmd":[py, str(self.root/"bridge"/"jarvis_voice_server.py")], "cwd":self.root/"bridge", "timeout":90},
        }
        self.processes = {}
    ...
    def _health(self, name):
        s=self.specs[name]
        try:
            req=urllib.request.Request(f"http://127.0.0.1:{s['port']}{s['health']}", headers={"Accept":"application/json"})
    ...
    def status(self, name):
        s=self.specs[name]
        return {"service":name,"port":s["port"],"port_open":tcp_check("127.0.0.1",s["port"]),"healthy":self._health(name)}

    def _tail(self,p,n=40):
        if not p.exists(): return "log não criado"
        return "\\n".join(p.read_text(encoding="utf-8",errors="replace").splitlines()[-n:])

    def start(self,name):
        s=self.specs[name]
        if self._health(name): return {"ok":True,"service":name,"status":"already_online","port":s["port"]}
        entry=Path(s["cmd"][1])
        if not entry.exists(): return {"ok":False,"service":name,"status":"entrypoint_missing","entrypoint":str(entry)}
        logp=self.runtime/f"{name}.log"
        log=open(logp,"a",encoding="utf-8",errors="replace")
        log.write(f"\\n===== START {name} {utc_now()} =====\\n"); log.flush()
        kwargs={"cwd":str(s["cwd"]),"stdout":log,"stderr":subprocess.STDOUT,"shell":False}
        if os.name=="nt": kwargs["creationflags"]=getattr(subprocess,"CREATE_NO_WINDOW",0x08000000)
        proc=subprocess.Popen(s["cmd"],**kwargs); self.processes[name]=proc
        deadline=time.time()+s["timeout"]
        while time.time()<deadline:
            if self._health(name): return {"ok":True,"service":name,"status":"started","pid":proc.pid,"port":s["port"]}
            if proc.poll() is not None: return {"ok":False,"service":name,"status":"process_exited","exit_code":proc.returncode,"log":self._tail(logp)}
            time.sleep(1)
        return {"ok":False,"service":name,"status":"health_timeout","pid":proc.pid,"port":s["port"],"log":self._tail(logp)}

    def start_stack(self):
        out=[]
        for name in ("engine","bridge","voice"):
            r=self.start(name); out.append(r)
            if not r.get("ok"): return {"ok":False,"failed":name,"results":out}
...
class Halem:
    def __init__(self):
        ...
        self.services = AuraServiceManager(AURA_ROOT)
    ...
    def intent(self, raw: str):
        text = normalize(raw)
        if text in {"sair", "exit", "quit"}: return "exit", None
        if text in {"chat", "conversar"}: return "chat", None
        if text in {"cancelar", "cancela", "nao", "não", "pare", "parar"}: return "cancel", None
        # Confirmação sempre vem antes de qualquer outro intent.
        if text.startswith("confirmar "):
            return "confirm", text[10:].strip()
        if self.pending_action and text in {"sim","s","confirmo","confirmado","confirmada","pode","pode sim","prossiga","continue","continuar","ok","okay","certo","isso","execute","executar"}:
            return "confirm", self.pending_action

        starts=("inicie","iniciar","ligue","ligar","ative","ativar","execute","executar","rode","rodar")
        services=("servicos","serviços","servico","serviço","tudo","stack","sistema")
        opens=("abra","abrir","mostrar","exibir")
        ui=("aura","desktop","interface","matriz")
        if any(x in text for x in starts) and any(x in text for x in services) and any(x in text for x in opens) and any(x in text for x in ui):
            return "action", "iniciar stack e abrir aura"
        if any(x in text for x in starts) and any(x in text for x in services):
            return "action", "iniciar stack"
        if any(x in text for x in opens) and any(x in text for x in ui):
            return "action", "abrir aura"
        for typo, svc in {"brigde":"bridge","bridge":"bridge","voz":"voice","voice":"voice","motor":"engine","engine":"engine"}.items():
            if typo in text:
                if any(x in text for x in ("reiniciar","reinicie","restart")): return "action", f"reiniciar {svc}"
                if any(x in text for x in starts): return "action", f"iniciar {svc}"
        if any(x in text for x in ("auditar aura","faça auditoria","faca auditoria","auditoria","audite o sistema")): return "action", "auditar aura"
        if text in {"status","estado","diagnostico","diagnóstico","diagnostico completo","diagnóstico completo","esta pronto","está pronto","está funcionando","esta funcionando"}: return "status", None
        if len(raw)>=900 or "# role" in text and "# objective" in text: return "full_spec", raw.strip()
        if any(x in text for x in ("manutenção","manutencao")): return "action", "manutencao segura"
        if any(x in text for x in ("corrija a aura","corrigir a aura","repare a aura","reparar a aura","corrija isso")): return "action", "corrigir auditavel"
        if any(x in text for x in ("desative a aura","desativar a aura","desligue a aura","desligar a aura")): return "action", "desativar segura"
        if any(x in text for x in ("reinicie a aura","reiniciar a aura","reinicie tudo","reiniciar tudo","reiniciar o sistema completo")): return "action", "reiniciar aura"
        if any(x in text for x in ("abrir arquivo","abra o arquivo","ler arquivo","leia o arquivo","carregar arquivo","analise o arquivo","analisar o arquivo")): return "read_file", raw.strip()
        if text in {"ajuda","help","comandos"}: return "help", None
        if text in {"reparar","diagnosticar reparo","plano de reparo"}: return "repair_plan", None
        return "model", None
    ...
    def ask_confirmation(self, action: str):
        ...
        text = f"⚠️ AÇÃO PENDENTE\n{description}\n\nNada foi executado. Responda SIM, CONFIRMO ou digite:\nCONFIRMAR {action.upper()}\n\nPara cancelar: NÃO ou CANCELAR."
        return text
    ...
    def _select_desktop_entrypoint(self):
        found=self.discover_aura_desktop()
        if not found: return None
        # Só autoexecuta lançadores claramente nomeados; arquivos genéricos ficam para inspeção.
        for p in found:
            n=p.name.lower()
            if p.suffix.lower() in {".exe",".bat",".cmd"} and ("aura" in n or n.startswith("start_aura") or n.startswith("launch_aura") or "desktop" in n):
                return p
        return None

    def _launch_entrypoint(self,p):
        if p.suffix.lower()==".exe": subprocess.Popen([str(p)],cwd=str(p.parent),shell=False)
        elif p.suffix.lower() in {".bat",".cmd"}: subprocess.Popen(["cmd.exe","/c",str(p)],cwd=str(p.parent),shell=False)
        else: raise RuntimeError(f"tipo não executável: {p.suffix}")

    def discover_aura_desktop(self):
        """Procura entrypoints plausíveis sem executar nada. Prioriza lançadores explícitos."""
        candidates=[]
        root=self.root if hasattr(self,'root') else AURA_ROOT
        patterns=["START_AURA*.bat","start_aura*.bat","launch_aura*.bat","start*.bat","launch*.bat","*.cmd","AURA*.exe","aura*.exe","*desktop*.exe","*dashboard*.exe","*matrix*.exe","*matriz*.exe","package.json","index.html","main.py","app.py"]
        skip={"venv",".git","node_modules","__pycache__",".tts_cache","site-packages"}
        for pat in patterns:
            try:
        ...
        # remove duplicados e ordena por maior probabilidade de ser launcher
        uniq=[]; seen=set()
        for p in candidates:
        ...
        def score(p):
            n=p.name.lower(); s=0
            if n.startswith("start") or n.startswith("launch"): s+=100
            if "aura" in n: s+=60
            if "desktop" in n or "dashboard" in n or "matrix" in n or "matriz" in n: s+=40
            if p.suffix.lower() in {".exe",".bat",".cmd"}: s+=30
            return -s
        return sorted(uniq,key=score)

    async def execute(self, action: str):
        if action == "iniciar stack":
            result=await asyncio.to_thread(self.services.start_stack); audit("stack_start",result=result)
            if result.get("ok"):
                return "🚀 AURA STACK ONLINE\n"+"\n".join(f"🟢 {r['service'].title()} · porta {r['port']} · {r['status']}" for r in result['results'])
            d=result['results'][-1]; return f"🔴 Falha em {result.get('failed')}: {d.get('status') or d.get('error')}\n📄 {d.get('log','')}"
        if action == "iniciar stack e abrir aura":
            result=await asyncio.to_thread(self.services.start_stack); audit("stack_start_then_open",result=result)
            if not result.get("ok"):
                d=result['results'][-1]; return f"🔴 Stack não ficou online. Falha: {result.get('failed')}: {d.get('status') or d.get('error')}\n📄 {d.get('log','')}"
            lines=["🚀 AURA STACK ONLINE"]+[f"🟢 {r['service'].title()} · porta {r['port']} · {r['status']}" for r in result['results']]
            target=await asyncio.to_thread(self._select_desktop_entrypoint)
            if target:
                try:
                    self._launch_entrypoint(target); lines.append(f"🖥️ AURA aberta: {target}")
                except Exception as e: lines.append(f"🔴 Falha ao abrir AURA: {e}")
            else: lines.append("🟡 Stack online, mas nenhum entrypoint de Desktop AURA foi identificado com segurança.")
            return "\n".join(lines)
        if action in {"iniciar engine","iniciar bridge","iniciar voice"}:
            name=action.split()[-1]; r=await asyncio.to_thread(self.services.start,name); audit("service_start",service=name,result=r)
            return (f"🟢 {name.title()} · porta {r['port']} · {r['status']}" if r.get('ok') else f"🔴 {name.title()} falhou: {r.get('status') or r.get('error')}\n📄 {r.get('log','')}")
        if action in {"reiniciar engine","reiniciar bridge","reiniciar voice"}:
            name=action.split()[-1]; proc=self.services.processes.get(name)
            if proc and proc.poll() is None:
                proc.terminate();
                try: proc.wait(timeout=8)
                except subprocess.TimeoutExpired: proc.kill()
            r=await asyncio.to_thread(self.services.start,name)
            return f"🟢 {name.title()} reiniciado." if r.get('ok') else f"🔴 Falha ao reiniciar {name}: {r}"
        if action == "abrir aura":
            target=await asyncio.to_thread(self._select_desktop_entrypoint)
            if not target:
                found=await asyncio.to_thread(self.discover_aura_desktop)
                if found:
                    sample="\n".join(str(p) for p in found[:20])
                    return "🟡 Não executei nenhum arquivo automaticamente porque o entrypoint não foi validado.\nPossíveis candidatos encontrados:\n"+sample
                return "🔴 Nenhum entrypoint de Desktop AURA encontrado em C:\\aura."
            try:
                self._launch_entrypoint(target); return f"🖥️ AURA aberta: {target}"
            except Exception as e: return f"🔴 Falha ao abrir AURA: {e}"
    ...
    async def handle(self, raw: str):
        ...
        if kind == "confirm":
            if self.pending_action and normalize(value or "") == normalize(self.pending_action):
                action=self.pending_action
                self.pending_action=None
                return await self.execute(action)
            return f"❌ Confirmação não corresponde à ação pendente. Pendente: {self.pending_action or 'nenhuma'}."
        snapshot = await asyncio.to_thread(collect_snapshot)
        async with self.model_lock:
