...
(() => {
  ...
  const root = $('auraApp');
  const state = { snapshot: null, analysis: null, charts: null, agents: null, activation: null, selectedGraph: 'appm', paused: false, frame: 0 };
  ...
  function agentMatches(agent, query, layer) {
    const hay = [agent.id, agent.name, agent.file, agent.layer, ...(agent.functions || [])].join(' ').toLowerCase();
    return (!query || hay.includes(query)) && (layer === 'all' || agent.layer === layer);
  }
  function renderAgents() {
    const box = $('matrixAgents'); if (!box) return;
    const all = Array.isArray(state.agents?.agents) ? state.agents.agents : [];
    const query = String($('agentSearch')?.value || '').trim().toLowerCase();
    const layer = String($('agentLayerFilter')?.value || 'all');
    const agents = all.filter(agent => agentMatches(agent, query, layer));
    const runnable = agents.filter(agent => agent.implementation_state === 'runnable').length;
    show('agentCount', `${agents.length}/${all.length || 34} agentes · ${runnable} executáveis`);
    if (!agents.length) {
      box.innerHTML = '<div class="matrix-agent-empty">Nenhum agente corresponde ao filtro. O catálogo continua preservado.</div>';
      return;
    }
    box.innerHTML = agents.map(agent => {
      const funcs = Array.isArray(agent.functions) ? agent.functions : [];
      const runnableFuncs = Array.isArray(agent.runnable_functions) ? agent.runnable_functions : [];
      const actions = Array.isArray(agent.actions) ? agent.actions : ['status', 'inspect'];
      const impl = String(agent.implementation_state || 'inspect_only');
      const stateLabel = impl === 'runnable' ? 'EXECUTÁVEL' : impl === 'source_missing' ? 'AUSENTE' : impl === 'syntax_error' ? 'SINTAXE' : impl === 'asset' ? 'ASSET' : 'INSPEÇÃO';
      const options = runnableFuncs.length ? runnableFuncs.map(fn => `<option value="${esc(fn)}">${esc(fn)}</option>`).join('') : '<option value="" disabled selected>Nenhuma função executável</option>';
      return `<article class="matrix-agent-card" data-agent-id="${esc(agent.id)}">
        <div class="matrix-agent-head"><div><small>${esc(agent.layer || 'sem camada')}</small><h3>${esc(agent.name || agent.id)}</h3></div><span class="status-chip ${impl === 'runnable' ? '' : 'matrix-agent-muted'}">${stateLabel}</span></div>
        <code>${esc(agent.file || '')}</code>
        <p class="matrix-agent-meta">${funcs.length} função(ões) · ${runnableFuncs.length} executável(eis) · ${esc(agent.source?.kind || 'sem fonte')}</p>
        <div class="matrix-agent-actions">${actions.filter(action => action !== 'run_function').map(action => `<button type="button" class="matrix-agent-action" data-action="${esc(action)}">${esc(action.replaceAll('_', ' '))}</button>`).join('')}</div>
        <details><summary>Funções e execução controlada</summary><select class="matrix-agent-function" aria-label="Função do agente">${options}</select><textarea class="matrix-agent-args" rows="2" spellcheck="false" placeholder="Argumentos JSON">{}</textarea><button type="button" class="matrix-agent-run" ${runnableFuncs.length ? '' : 'disabled'}>Executar em paper trade</button><div class="matrix-agent-functions">${funcs.map(fn => `<span class="${runnableFuncs.includes(fn) ? 'runnable' : 'inspect-only'}">${esc(fn)}${runnableFuncs.includes(fn) ? '' : ' · inspeção'}</span>`).join('') || 'Nenhuma função descoberta.'}</div></details>
        <pre class="matrix-agent-result" aria-live="polite"></pre>
      </article>`;
    }).join('');
    box.querySelectorAll('.matrix-agent-action').forEach(button => button.addEventListener('click', async () => {
      const card = button.closest('[data-agent-id]'); const result = card?.querySelector('.matrix-agent-result');
      if (!card || !result) return;
      button.disabled = true; result.textContent = 'A consultar o Engine...';
      try {
        const agentId = encodeURIComponent(card.dataset.agentId);
        const isReview = button.dataset.action === 'glm_review';
        const endpoint = isReview ? `/api/agents/${agentId}/glm-review` : `/api/agents/${agentId}/action`;
        const body = isReview ? { reason: 'Revisão advisory solicitada pela interface Matriz', context: { snapshot: state.snapshot, analysis: state.analysis } } : { action: button.dataset.action, payload: {} };
        const data = await A.request(endpoint, { method: 'POST', body });
        result.textContent = JSON.stringify(data, null, 2);
      } catch (error) { result.textContent = `Falha: ${error?.message || error}`; }
      finally { button.disabled = false; }
    }));
    box.querySelectorAll('.matrix-agent-run').forEach(button => button.addEventListener('click', async () => {
      const card = button.closest('[data-agent-id]'); const result = card?.querySelector('.matrix-agent-result'); const select = card?.querySelector('.matrix-agent-function'); const args = card?.querySelector('.matrix-agent-args');
      if (!card || !result || !select || !select.value) return;
      let call = {};
      try { call = args?.value.trim() ? JSON.parse(args.value) : {}; if (!call || typeof call !== 'object' || Array.isArray(call)) throw new Error('os argumentos devem ser um objeto JSON'); }
      catch (error) { result.textContent = `Argumentos inválidos: ${error.message}`; return; }
      button.disabled = true; result.textContent = `A executar ${select.value} em paper trade...`;
      try {
        const data = await A.request(`/api/agents/${encodeURIComponent(card.dataset.agentId)}/action`, { method: 'POST', body: { action: 'run_function', payload: { function: select.value, call } } });
        result.textContent = JSON.stringify(data, null, 2);
      } catch (error) { result.textContent = `Falha: ${error?.message || error}`; }
      finally { button.disabled = false; }
    }));
  ...
  async function loadAgents() {
    state.agents = await A.getAgents();
    renderAgents();
  }
  function bindAgentControls() {
    $('agentSearch')?.addEventListener('input', renderAgents);
    $('agentLayerFilter')?.addEventListener('change', renderAgents);
    $('refreshAgents')?.addEventListener('click', loadAgents);
  }
  function renderActivation() {
    const box = $('matrixTools'); if (!box) return;
    const tools = Array.isArray(state.activation?.tools) ? state.activation.tools : [];
    box.innerHTML = tools.length ? tools.map(tool => `<span class="matrix-tool-chip ${tool.enabled ? 'enabled' : 'disabled'}">${esc(tool.name)} · ${tool.enabled ? 'ATIVA' : 'INATIVA'}</span>`).join('') : '<span class="matrix-tool-chip disabled">Ferramentas aguardando o Engine</span>';
  }
  async function loadActivation() {
    state.activation = await A.getActivation();
    renderActivation();
  }
  function appendChatMessage(text, role) {
    const box = $('matrixChatBox'); if (!box) return;
    const row = document.createElement('div'); row.className = `matrix-chat-message ${role}`; row.textContent = String(text || ''); box.appendChild(row); box.scrollTop = box.scrollHeight;
  }
  async function sendMatrixChat(event) {
    event.preventDefault();
    const input = $('matrixChatInput'); const stateLabel = $('matrixChatState');
    const message = String(input?.value || '').trim(); if (!message) return;
    appendChatMessage(message, 'user'); input.value = ''; if (stateLabel) { stateLabel.textContent = 'A RESPONDER'; stateLabel.classList.add('pending'); }
    const data = await A.sendChat({ message, fixtureId: fixtureId(), context: state.snapshot, systemContext: { view: 'matrix', mode: 'PLAN_ONLY', failClosed: true, execution_allowed: false }, history: [] });
    if (data?.reply) { appendChatMessage(data.reply, 'assistant'); if (stateLabel) { stateLabel.textContent = data.model || 'GLM PRONTO'; stateLabel.classList.remove('pending'); } }
    else { appendChatMessage(`GLM indisponível: ${data?.error || 'verifique Ollama e o modelo configurado.'}`, 'assistant'); if (stateLabel) { stateLabel.textContent = 'INDISPONÍVEL'; stateLabel.classList.add('matrix-agent-muted'); } }
    if (input) input.focus();
  }
  function bindMatrixChat() { $('matrixChatForm')?.addEventListener('submit', sendMatrixChat); }
  function renderAll(){renderHeader();renderRisk();renderCorners();renderGraphTabs();drawGraphPreview();renderContext();renderFeatures();renderModels();renderQuality();renderTrace();renderLedger();drawField();renderAgents();renderActivation();}
  ...
  async function boot(){buildLayoutMenu();bindPanels();bindControls();bindAgentControls();bindMatrixChat();await refresh();await loadAgents();await loadActivation();refreshVoiceHealth();loop();setInterval(refresh,4000);setInterval(loadAgents,30000);setInterval(loadActivation,30000);setInterval(refreshVoiceHealth,15000);}
  boot();
})();
