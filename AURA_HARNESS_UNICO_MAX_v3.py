...
async def ask_ollama(user_text: str, snapshot: dict) -> str:
    system = (
        ...
        "Não repita diagnósticos, listas ou recomendações genéricas. Máximo de 8 linhas, salvo quando o usuário pedir análise detalhada. Seja específico: diga o que detectou, o que fez, o que não fez e qual é o próximo passo."
    )
    payload = {
        "model": MODEL,
        "messages": [
...
# ============================================================
# Workspace / arquivos — leitura ampla, mutação sempre com backup
# ============================================================
TEXT_EXTS = {".txt",".md",".json",".jsonl",".yaml",".yml",".ini",".cfg",".conf",".toml",".csv",".log",".py",".ps1",".bat",".cmd",".js",".ts",".tsx",".jsx",".html",".css",".xml",".sql"}

def workspace_scan():
    rows=[]
    for p in AURA_ROOT.rglob("*"):
        if not p.is_file(): continue
        low=[x.lower() for x in p.parts]
        if any(x in {".git","node_modules","venv","__pycache__","site-packages"} for x in low): continue
        try:
            st=p.stat(); rows.append({"path":str(p),"ext":p.suffix.lower(),"size":st.st_size,"mtime":st.st_mtime})
        except OSError: pass
    return rows

def backup_paths(paths, label="operation"):
    stamp=time.strftime("%Y%m%d_%H%M%S")
    root=BACKUP_ROOT/f"{label}_{stamp}"
    root.mkdir(parents=True,exist_ok=True)
    copied=[]
    aura=AURA_ROOT.resolve()
    for raw in paths:
        p=Path(raw).resolve()
        try: rel=p.relative_to(aura)
        except ValueError: continue
        if p.is_file():
            dest=root/rel; dest.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(p,dest); copied.append(str(p))
    audit("backup_created", label=label, backup=str(root), files=copied)
    return root,copied

def safe_extract_zip(zip_path):
    import zipfile
    z=Path(zip_path).resolve()
    if not z.is_file() or z.suffix.lower() != ".zip": raise ValueError("ZIP inválido")
    dest=z.with_suffix(""); dest.mkdir(exist_ok=True)
    base=dest.resolve()
    with zipfile.ZipFile(z) as f:
        for m in f.infolist():
            target=(base/m.filename).resolve()
            if not str(target).lower().startswith(str(base).lower()+os.sep): raise ValueError("ZIP inseguro: caminho fora do destino")
        f.extractall(base)
    audit("zip_extracted", source=str(z), destination=str(base))
    return base

def safe_text_read(path, limit=120000):
    p=Path(path).resolve()
    if AURA_ROOT.resolve() not in p.parents and p != AURA_ROOT.resolve(): raise ValueError("arquivo fora de C:\\aura")
    if p.suffix.lower() not in TEXT_EXTS: raise ValueError("extensão não textual")
...
class Halem:
    ...
    def intent(self, raw: str):
        ...
        # Confirmation must be evaluated before every other intent.
        if text.startswith("confirmar "): return "confirm",text.removeprefix("confirmar ").strip()
        if self.pending_action and text in {"sim","s","confirmo","confirmado","confirmada","pode","pode sim","prossiga","continue","continuar","ok","okay","certo","isso","execute","executar"}: return "confirm",self.pending_action
        if len(raw)>=900 or ("# role" in text and "# objective" in text): return "full_spec",raw.strip()
        if text in {"status","estado","diagnostico","diagnóstico","diagnostico completo","diagnóstico completo","saude","saúde","como esta","como está"}: return "status",None
        # Composite start/open commands.
        starts=("inicie","iniciar","ligue","ligar","ative","ativar","execute","executar","rode","rodar","suba","subir")
        service_terms=("servico","serviço","servicos","serviços","tudo","stack","sistema","engine","bridge","voice")
        opens=("abra","abrir","mostrar","exibir","inicie a interface","iniciar a interface")
        ui=("aura","desktop","interface","matriz","matrix","dashboard")
        if any(x in text for x in starts) and any(x in text for x in service_terms) and any(x in text for x in opens) and any(x in text for x in ui): return "action","iniciar stack e abrir aura"
        if any(x in text for x in starts) and any(x in text for x in ("tudo","stack","todos os servicos","todos os serviços","sistema aura","todo sistema","todos servicos","todos serviços")): return "action","iniciar stack"
        if any(x in text for x in opens) and any(x in text for x in ui): return "action","abrir aura"
        ...
        if any(x in text for x in ("listar arquivos","liste arquivos","inventario de arquivos","inventário de arquivos","varrer pasta","analisar pasta","organizar arquivos")): return "action","auditar arquivos"
        if any(x in text for x in ("extrair zip","descompactar zip","abrir zip")): return "action","extrair zip"
        for typo,svc in {"brigde":"bridge","bridg":"bridge","bridge":"bridge","voz":"voice","voice":"voice","motor":"engine","engine":"engine"}.items():
            if typo in text:
        ...
        if any(x in text for x in ("manutenção","manutencao","verificação de manutenção","verificacao de manutencao")): return "action","manutencao segura"
        ...
        if any(x in text for x in ("instalar agente","instale um agente","baixar agente","adicionar agente","instala um agente")): return "agent_installer",raw.strip()
        if any(x in text for x in ("criar novos agentes","construir novos agentes","criar um agente","construir um agente","novo agente","novos agentes")): return "agent_builder",raw.strip()
    ...
    def find_desktop_entrypoint(self):
        candidates=[]
        skip={".git","node_modules","venv","__pycache__","site-packages"}
        for p in AURA_ROOT.rglob("*"):
            if not p.is_file() or any(x.lower() in skip for x in p.parts): continue
            n=p.name.lower(); ext=p.suffix.lower()
            score=0
            if ext in {".exe",".bat",".cmd"}: score+=20
            if "aura" in n: score+=50
            if any(k in n for k in ("desktop","dashboard","matrix","matriz","launcher","start","launch")): score+=30
            if score>=70: candidates.append((score,p))
        return max(candidates,key=lambda x:x[0])[1] if candidates else None

    def launch_desktop(self,p):
        ...
        else: raise ValueError("entrypoint não executável")

    async def execute(self, action: str):
        if action == "auditar arquivos":
            rows=await asyncio.to_thread(workspace_scan)
            byext={}
            for r in rows: byext[r["ext"] or "[sem extensão]"]=byext.get(r["ext"] or "[sem extensão]",0)+1
            largest=sorted(rows,key=lambda x:x["size"],reverse=True)[:10]
            lines=[f"📂 Arquivos analisados: {len(rows)}", "Tipos: "+", ".join(f"{k}={v}" for k,v in sorted(byext.items(),key=lambda x:-x[1])[:15]), "Maiores arquivos:"]
            lines += [f"  {Path(r['path']).name} · {r['size']/1024/1024:.1f} MB" for r in largest]
            audit("workspace_audit", files=len(rows), extensions=byext)
            return "\n".join(lines)+"\n🔒 Nenhum arquivo alterado."
        if action == "extrair zip":
            return "⚠️ Para extrair com segurança, informe o caminho do ZIP dentro de C:\\aura. A extração usa validação contra Zip Slip e não executa instaladores."
        if action == "iniciar stack":
            # Prefer the real safe activation entrypoint already shipped by the project.
            return await self.run_safe_activation()
        if action == "iniciar stack e abrir aura":
            result=await self.run_safe_activation()
            target=await asyncio.to_thread(self.find_desktop_entrypoint)
            if target:
                try: await asyncio.to_thread(self.launch_desktop,target); return result+f"\n🖥️ Desktop AURA iniciado: {target}"
                except Exception as exc: return result+f"\n🔴 Falha ao abrir Desktop: {exc}"
            return result+"\n🟡 Stack tratado pela ativação segura, mas nenhum Desktop/Matrix oficial foi identificado."
        if action.startswith("iniciar ") and action.split()[-1] in {"engine","bridge","voice"}:
            svc=action.split()[-1]; return f"ℹ️ {svc.upper()} faz parte da ativação segura do stack. Use 'inicie todos os serviços' para manter a ordem oficial."
