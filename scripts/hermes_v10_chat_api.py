...
# -*- coding: utf-8 -*-
"""
Hermes V10 Ultra — Chat API v2 (Final Integrada)
...
def sanitize_payload(data):
    """Converte tipos numpy/não-JSON em tipos Python nativos."""
    try:
        import numpy as np
        _np = True
    except Exception:
...
# ─── Métricas ────────────────────────────────────────────────────────────────
REQUEST_COUNT = Counter("hermes_requests_total", "Total requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("hermes_request_duration_seconds", "Request latency", ["endpoint"])
WS_CONNECTIONS = Counter("hermes_websocket_connections_total", "WebSocket connections accepted")
WS_ACTIVE = Gauge("hermes_websocket_connections", "Active WebSocket connections")
WS_MESSAGES = Counter("hermes_websocket_messages_total", "WebSocket messages")
MEMORY_OPS = Counter("hermes_memory_ops_total", "Memory operations", ["op"])

# ─── Logger ──────────────────────────────────────────────────────────────────
try:
    import structlog
    logger = structlog.get_logger("hermes.chat_api")
except ImportError:
    import logging
    logging.basicConfig(level=logging.INFO)
    logger = logging.getLogger("hermes.chat_api")
...
def _has_auth_token(message: str) -> bool:
    ...
    if re.search(r"\b(n[aã]o|nunca|jamais|proib)\w*\s+(autoriz|permit)\w*", low):
        return False
    return bool(re.search(r"\b(autorizo|autorizado|eu\s+autorizo|com\s+autoriza[cç][aã]o)\b", low))


def _strip_auth_token(message: str) -> str:
    return re.sub(
        r"\b(autorizo|autorizado|eu\s+autorizo|com\s+autoriza[cç][aã]o)\b[:\s-]*",
        " ",
        message or "",
        flags=re.IGNORECASE,
    ).strip()
...
async def lifespan(app: FastAPI):
    ...
    try:
        engine = HermesLLMEngine(
            ...
            ollama_model=os.getenv("OLLAMA_MODEL") or os.getenv("AURA_OLLAMA_MODEL") or os.getenv("AURA_JARVIS_MODEL") or "qwen2.5:3b-instruct",
    ...
    if engine is not None:
        try:
            ...
            engine.register_tool("read_file", tool_read_file, "Lê arquivo", {
                "type": "object", "properties": {"path": {"type": "string"}, "root": {"type": "string"}},
                "required": ["path"],
            })
            engine.register_tool("list_dir", tool_list_dir, "Lista diretório", {
                "type": "object", "properties": {"path": {"type": "string"}, "root": {"type": "string"}},
            })
            ...
            engine.register_tool("check_constitution", tool_check_constitution, "Verifica constituição", {
    ...
    try:
        ...
        # ─── AURA GYM — auto-treino no primeiro boot + refresh ─────────────
        ...
        try:
            ...
            # Scheduler diário ~06:00 (refresh, não bloqueia event loop)
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    # localhost / operador local — sem rate limit agressivo (evita Failed to fetch)
...
async def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> str:
    ...
    # Simples HMAC verification (em produção, use JWT library)
...
def _system_snapshot():
    ...
    ollama_ok = False
    ollama_ms = None
    ollama_model = os.getenv("AURA_OLLAMA_MODEL") or os.getenv("AURA_JARVIS_MODEL") or os.getenv("CORNERAI_CHAT_MODEL") or "qwen2.5:3b-instruct"
    try:
        t0 = time.time()
        _u = _urllib()
        req = _u.Request(
        ...
        try:
            ...
            if names:
                want = (os.getenv("AURA_OLLAMA_MODEL") or os.getenv("AURA_JARVIS_MODEL") or os.getenv("OLLAMA_MODEL") or "qwen2.5:3b-instruct")
...
def _cached_anomaly(max_age: float = 60.0):
    """detect() no maximo 1x/min — impede poll do dashboard de poluir o historico."""
    now = time.time()
...
async def health_live():
    """Liveness puro — nunca falha, sem anomalia/psutil."""
...
def _sanitize_llm(text: str, message: str) -> str:
    ...
    low = raw.lower()
    bad = (
        "nao posso ajudar", "não posso ajudar", "desculpe, mas não posso",
        "credenciais", "/hermes on", "gerenciador de tarefas",
        "não posso fornecer", "nao posso fornecer", "suporte técnico",
        "suporte tecnico", "caminho de sistema", "c:\\usr\\local",
        "unix/linux", "atualize o sistema aura", "versão mais recente",
    ...
    if cat.exists():
        try:
            ...
            for e in iterable:
                ...
                lines.append(f"{e.get('code')}: {e.get('title')} → {action}")
...
async def chat(req: ChatMessage):
    ...
    # ── CAMADA 0 — Operador humano WhatsApp/Telegram (macros fora do event loop) ──
    try:
    ...
    if error_catalog and mfix:
        ...
        if not _has_auth_token(msg):
            ...
            reply = (
                f"{code} — simulacao (dry-run). Nao apliquei no sistema vivo.\n"
    ...
    # Comando PURO: frase curta só com a ação (ex: "status", "reinicia engine").
    # Perguntas longas com a palavra "status" vão para o LLM (conversa).
    ...
    PROBLEM_RE = re.compile(
        r"(ca[ií]u|caiu|offline|fora do ar|n[aã]o (?:funciona|responde|abre|carrega|sobe)|"
    ...
    def _sanitize_reply(text: str, mdl: str) -> str:
        ...
        if m:
            ...
            if _re.search(r"n[ãa]o\s+(sou|sou\s+o|perten[cç]o|fui)", window, _re.IGNORECASE):
                return s  # negacao honesta — nao censurar
            return (
    ...
    try:
        # CAMADA 1 — COMANDO DIRETO (so se for comando puro)
        # cancelar confirmacao pendente
        if low.strip() in ("cancelar", "cancela", "cancel", "nao autorizo", "não autorizo"):
        ...
        # AUTONOMIA (maestro) — kill switch + relatorio
        ...
        if low.strip() in ("autonomia", "relatorio autonomia", "relatório autonomia") or re.match(r"^\s*autonomia\s*$", low):
        ...
        if low.strip() in ("modo manutencao", "modo manutenção", "entra manutencao", "entra manutenção") or re.match(
            r"^\s*(modo\s+manuten[cç][aã]o|entra\s+manuten[cç][aã]o)\s*$", low
        ):
        ...
        if low.strip() in ("sai do modo manutencao", "sai do modo manutenção", "sai manutencao", "sai manutenção") or re.match(
            r"^\s*sai\s+(do\s+)?modo\s+manuten[cç][aã]o\s*$", low
        ):
        ...
        if low.strip() in ("postmortem", "post-mortem", "ultimo postmortem", "último postmortem") or re.match(
        ...
        # AURA GYM — treino offline no simulador (nunca no sistema vivo)
        ...
        if _is_pure_command(msg) or _op_extra:
            ...
            if destructive and not _has_auth_token(msg):
                ...
                model = "aura:gpu"
            elif re.match(r"^\s*(?:autori[sz][oa]\s+|pode\s+|podes\s+)?(diagn[oó]stic|diag|an[aá]lise)", low):
...
async def correct(req: CorrectRequest, _auth: str = Depends(require_local_or_token)):
    ...
    if req.simulate_first and digital_twin:
        ...
        if hasattr(sim_result, "success") and not sim_result.success:
            if alerts:
                await _maybe_await(alerts.send(
                    "warning", "correct_endpoint", f"Simulação falhou para {req.code}",
...
async def simulate(req: SimulateRequest, _auth: str = Depends(require_local_or_token)):
    if not digital_twin:
        raise HTTPException(status_code=503, detail="Digital Twin não inicializado")
...
async def healing_log(limit: int = 50):
    if not healing:
        raise HTTPException(status_code=503, detail="Healing não inicializado")
...
async def memory_search(q: str, top_k: int = 5):
    if not memory:
        raise HTTPException(status_code=503, detail="Memory não inicializada")
...
async def alerts_unacknowledged(min_severity: str = "warning"):
    if not alerts:
        raise HTTPException(status_code=503, detail="Alert manager não inicializado")
    ...
    try:
        while True:
            ...
            try:
                ...
                if engine is None:
                    await websocket.send_json({"reply": "Ollama/LLM offline — use REST /api/chat.", "model": "fallback", "ok": False})
                    continue
...
<title>AURA Hermes · Operator OS verde</title>
<style>
...
<div class="wrap">
  <div class="top">
    <div class="brand">AURA QUANT-X<b>Hermes Operator · verde</b></div>
    <div class="pills">
  ...
  <div class="foot">paper_trade=true · execution_allowed=false · Ollama :11434 nunca e morto · v37.3.47-AUDIT</div>
</div>
<script>
...
async function sendMsg(){
  ...
  try{
    ...
    box.innerHTML += '<div class="msg bot">'+esc(d.reply)+'<span class="meta">'+(d.model||'')+' · '+Math.round(d.latency_ms||0)+'ms</span></div>';
  }catch(e){
