...
def _spawn_detached(title: str, inner_cmd: str, cwd: Path, argv: list = None,
                    stdout_path=None, stderr_path=None, env: dict = None) -> None:
    """Sai do Job Object do wrapper para o processo sobreviver ao BAT pai."""
    if os.name == "nt" and argv:
        LOG_DIR.mkdir(parents=True, exist_ok=True)
        outp = stdout_path or (LOG_DIR / "spawn.out.log")
        errp = stderr_path or (LOG_DIR / "spawn.err.log")
        out = open(outp, "a", encoding="utf-8")
        err = out if str(errp) == str(outp) else open(errp, "a", encoding="utf-8")
...
def _start_alfred(cfg: dict) -> None:
    ...
    _spawn_detached(
        "AURA-Alfred", inner, ROOT,
        argv=[sys.executable, "-m", "alfred.api", "--host", cfg["host"], "--port", str(cfg["port"])],
        stdout_path=ALFRED_STDOUT, stderr_path=ALFRED_STDERR,
    )
    _log("Alfred lançado (breakaway)")
...
def _start_hermes() -> None:
    ...
    inner = (
        ...
        f'set PAPER_TRADE={env.get("PAPER_TRADE","true")}& set EXECUTION_ALLOWED=false& set PYTHONUTF8=1& set AURA_NO_BROWSER=1& '
    ...
    _spawn_detached(
        "AURA-Hermes-Chat", inner, ROOT / "hermes_v10",
        argv=[sys.executable, "-u", str(script)],
        stdout_path=HERMES_LOG, stderr_path=HERMES_LOG, env=env,
    )
    _log("Hermes lançado (breakaway)")
...
def cmd_start() -> int:
    ...
    _check_python()
    _check_ollama(cfg)
    from .supervise import launch_supervisor
    sup = launch_supervisor()
    _log(f"supervisor {sup}")
    health_a = f"http://{cfg['host']}:{cfg['port']}/health"
    if not _wait_health(health_a, 30):
        raise SystemExit(f"Alfred não respondeu em {health_a}. Log: {ALFRED_STDERR}")
    if not _wait_health("http://127.0.0.1:8777/health", 40):
        raise SystemExit(f"Hermes não respondeu em :8777. Log: {HERMES_LOG}")
    _log("arranque concluído — supervisor a manter Alfred+Hermes; browser NÃO aberto")
...
def cmd_stop() -> int:
    cfg = get_config()
    from .supervise import PID_PATH as SUP_PID
    try:
        spid = int(SUP_PID.read_text(encoding="utf-8").strip())
        _kill_pid(spid, "supervisor")
