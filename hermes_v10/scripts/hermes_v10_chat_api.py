...
def _live_paper_trade() -> bool:
    try:
        p = Path(r"C:\aura\config\aura_flags.json")
        if p.exists():
            return bool(json.loads(p.read_text(encoding="utf-8")).get("paper_trade", True))
    except Exception:
        pass
    return os.getenv("PAPER_TRADE", "true").lower() == "true"


def _live_repair() -> bool:
    try:
        p = Path(r"C:\aura\config\aura_flags.json")
        if p.exists():
            return bool(json.loads(p.read_text(encoding="utf-8")).get("system_repair_allowed", False))
    except Exception:
        pass
    return False
...
async def health():
    ...
    payload = {
        ...
        "ok": True,
        "paper_trade": bool(_live_paper_trade()),
        "execution_allowed": False,
        "system_repair_allowed": bool(_live_repair()),
        "env_safe": bool(env_safe),
...
@app.get("/api/tools")
async def api_tools():
    """Catálogo de ferramentas Alfred (core + plugins). Sem abertura de browser."""
    import requests as _rq
    try:
        r = _rq.get("http://127.0.0.1:8791/tools", timeout=5)
        r.raise_for_status()
        data = r.json()
        data["alfred"] = True
        return data
    except Exception as exc:  # noqa: BLE001
        return {"alfred": False, "error": str(exc)[:200], "tools": []}


@app.post("/api/stt")
async def api_stt(req: Request):
    """Recebe WAV em base64, transcreve no Voice :8099 (Whisper pt)."""
    try:
        body = await req.json()
    except Exception:
        body = {}
    ...
    try:
        r = _rq.post(
            "http://127.0.0.1:8099/api/voice/stt",
            json={
                "audio_base64": str(body.get("audio_base64") or ""),
                "mime": str(body.get("mime") or "audio/wav"),
                "session_id": str(body.get("session_id") or "chat"),
            },
            timeout=60,
        )
        ...
        return {"ok": False, "error": str(exc)[:200],
                "nota": "Servidor de voz :8099 offline. Sobe AURA_SUBIR_VOZ.bat"}


@app.post("/api/listen")
async def api_listen(req: Request):
    """Grava o microfone do Windows (Realtek) no PC — não no browser — e transcreve."""
    try:
        body = await req.json()
    except Exception:
        body = {}
    ...
    try:
        r = _rq.post(
            ...
            json={
                ...
                "session_id": str(body.get("session_id") or "chat"),
                "prefer": str(body.get("prefer") or "realtek"),
            },
            timeout=90,
        )
        data = r.json() if r.content else {}
...
async def api_upload(req: Request):
    """Guarda ficheiros na inbox do Alfred (zip, py, md, json, imagem, áudio…)."""
    try:
        body = await req.json()
    except Exception:
        body = {}
    ...
    from pathlib import Path as _P
    inbox = _P(r"C:\aura\data\alfred\inbox")
    ...
    saved = []
    for item in (body.get("files") or [])[:8]:
        name = str((item or {}).get("name") or "ficheiro")
        name = _P(name).name
        name = re.sub(r"[^A-Za-z0-9._-]+", "_", name)[:120] or "ficheiro"
        ext = _P(name).suffix.lower()
        if ext not in allowed:
            saved.append({"name": name, "ok": False, "error": f"formato {ext} recusado"})
            continue
        raw = str((item or {}).get("b64") or "")
        if "," in raw and raw.lower().split(",", 1)[0].startswith("data:"):
            raw = raw.split(",", 1)[1]
        try:
            data = _b64.b64decode(raw, validate=False)
        except Exception:
            saved.append({"name": name, "ok": False, "error": "base64 inválido"})
            continue
        if not data or len(data) > 25 * 1024 * 1024:
            saved.append({"name": name, "ok": False, "error": "vazio ou >25 MB"})
            continue
        dest = inbox / name
        dest.write_bytes(data)
        saved.append({"name": name, "ok": True, "path": str(dest), "size": len(data)})
    ok_paths = [s["path"] for s in saved if s.get("ok") and s.get("path")]
    msg = str(body.get("message") or "").strip()
    if not msg and ok_paths:
        msg = "Alfred, processa o ficheiro " + ok_paths[0]
    alfred = None
    if msg:
        alfred = await asyncio.to_thread(_try_alfred_route, msg, str(body.get("session_id") or "upload"))
    return {"ok": any(s.get("ok") for s in saved), "saved": saved, "alfred": alfred,
            "nota": "Ficheiros na inbox C:\\aura\\data\\alfred\\inbox. AUTORIZO para instalar .py/.zip."}
...
async def api_speak(req: Request):
    ...
    import requests as _rq
    try:
        r = _rq.post("http://127.0.0.1:8791/voice/speak", json={"text": str(body.get("text") or "")[:500]},
                     timeout=70)
        return r.json()
    except Exception as exc:  # noqa: BLE001
        return {"ok": False, "error": str(exc)[:200]}
...
def _try_alfred_route(message: str, session_id: str):
    """Alfred ANTES do fallback conversacional. None = Hermes continua."""
    try:
        from alfred.hermes_adapter import handle_hermes_message
        return handle_hermes_message(message, session_id=session_id)
    except Exception as exc:  # noqa: BLE001
        logger.warning("alfred_route_skip error=%s", exc)
        low = (message or "").strip().lower()
        if low.startswith("alfred") or low.startswith("aura alfred") or low.startswith("hermes, executa") or low.startswith("hermes executa"):
            return {
                "route": "alfred",
                "model": "alfred:qwen3:8b",
                "reply": f"Alfred indisponível: {type(exc).__name__}: {exc}",
                "plan": {},
                "job_id": "",
...
async def chat(req: ChatMessage):
    ...
    alfred_out = await asyncio.to_thread(_try_alfred_route, msg, session_id)
    if alfred_out is not None:
        suggestions = (
            ["AUTORIZO", "CANCELA", "Alfred, estado"]
            if alfred_out.get("requires_confirmation")
            else ["Alfred, estado", "Alfred, ajuda"]
        )
        return ChatResponse(
            reply=str(alfred_out.get("reply") or ""),
            model=str(alfred_out.get("model") or "alfred:qwen3:8b"),
            ...
            route="alfred",
            plan=alfred_out.get("plan") or {},
            job_id=alfred_out.get("job_id") or "",
            requires_confirmation=bool(alfred_out.get("requires_confirmation")),
            status=alfred_out.get("status") or "",
            job=alfred_out.get("job"),
        )
    ...
    ACTION_RE = re.compile(
        ...
        r"liga(?:r)?|ligue|ative?|ativar)"
        r"(?:\s+(?:engine|bridge|matriz|hermes|voz|voice|all|tudo|desktop|o\s+desktop|a\s+matriz|"
        r"a\s+c[aâ]mera(?:\s+do\s+windows)?|todos(?:\s+os)?\s+agentes?|[0-9]{2,3}%?))?"
        r"\s*$",
        re.IGNORECASE,
    )
    ...
    try:
        ...
        _op_extra = any(k in low for k in (
            ...
            "ative todos", "ativar todos", "ativa todos", "ative todos agentes",
            "fecha e abre", "fechar e abrir", "reabre tudo",
            "liga a camera", "ligue a camera", "ligar a camera", "abre a camera",
        ))
        ...
        # CAMADA 3 â€” CONVERSA GERAL
            ...
            if engine:
                ...
                system = (
                    ...
                    "- Modelo exclusivo: qwen3:8b. Sem fallback silencioso para 1B/3B.\n"
                    ...
                    "Comandos: status, corrige, diagnostico, reinicia engine, abra desktop, gpu, "
                    "Alfred (prefixo), ligue a camera, o que consegue fazer, ative todos agentes.\n"
                    "paper_trade=true bloqueia APOSTAS reais. NAO bloqueia camera, desktop, SokkerPRO, "
                    "correcao de UI nem extração de dados. Sem ferramenta de casas de apostas.\n"
                    "Invariantes: execution_allowed=false. Nunca invente fixtures, odds ou servicos.\n"
                    "Camera e apps Windows: encaminhe para Alfred (open_app/capture_camera). "
                    "Nao diga que a camera nao esta registada.\n"
...
.send input,.send textarea{flex:1;background:#07080c;border:1px solid var(--line);color:var(--txt);padding:10px 12px;outline:none;font-family:ui-monospace,Consolas,monospace;font-size:13px;resize:vertical;min-height:48px}
.send input:focus,.send textarea:focus{border-color:var(--acid)}
...
<div class="wrap">
  ...
  <div class="chat">
    ...
    <div class="actions" id="suggestBox">
      <button type="button" data-msg="Alfred, desativa paper_trade">Paper-trade OFF</button>
      <button type="button" data-msg="Alfred, repara o aura">Reparar Aura</button>
      <button type="button" data-msg="Alfred, observa o pc">Ver PC</button>
      <button type="button" id="btnMicHint">Falar com o Alfred</button>
      <button type="button" data-msg="Alfred, estado">Alfred estado</button>
      <button type="button" data-msg="Alfred, ferramentas">Ferramentas</button>
      <button type="button" data-msg="Alfred, modo funcionario">Foco Alfred</button>
      <button type="button" data-msg="Alfred, abre photoshop">Photoshop</button>
      <button type="button" data-msg="Alfred, exporta worker gpu">Worker GPU</button>
      <button type="button" data-msg="Alfred, gpu">GPU</button>
      <button type="button" data-msg="Alfred, serviços">Serviços</button>
      <button type="button" data-msg="ligue a camera">Câmara</button>
      <button type="button" data-msg="o que consegue fazer">O que faz</button>
      <button type="button" data-msg="ative todos agentes">Ativar agentes</button>
      <button type="button" data-msg="status">Status</button>
      <button type="button" class="ghost" data-msg="diagnostico">Relatorio</button>
      <button type="button" class="ghost" data-msg="Alfred, instala esta ferramenta">Instalar ferramenta</button>
    </div>
    <div class="send">
      <textarea id="msgInput" rows="3" placeholder="Clica FALAR · ou envia zip/py/md/json/imagem/áudio · ou escreve aqui" onkeydown="if(event.key==='Enter' && !event.shiftKey){event.preventDefault();sendMsg()}"></textarea>
      <input type="file" id="fileInput" multiple accept=".txt,.md,.json,.py,.zip,.png,.jpg,.jpeg,.webp,.gif,.wav,.mp3,.ogg,.webm,.pdf,.csv,.bat,.ps1,.yml,.yaml,.html,.css,.js,.toml,.docx" style="display:none">
      <button type="button" class="ghost" onclick="document.getElementById('fileInput').click()">Ficheiro</button>
      <button type="button" id="btnMic" style="background:#1f6feb;min-width:7rem">Falar</button>
      <button type="button" onclick="sendMsg()">Enviar</button>
    </div>
  </div>
  <div class="foot" id="footFlags">paper_trade=? · execution_allowed=false · Ollama :11434 nunca e morto</div>
</div>
<script>
const API = (function(){const o=location.origin; if(/:(8778|8766|8099)$/.test(o)) return o.replace(/:(8778|8766|8099)$/, ':8777'); return o;}());
let lastModel='';
let lastReply='';
let sessionId = localStorage.getItem('aura_session_id') || ('sess_'+Math.random().toString(16).slice(2));
...
function renderPlan(d){
  ...
  let html='';
  if(d.route==='alfred' || tasks.length || d.job_id){
    html += '<div class="meta">route='+esc(d.route||'')+' · status='+esc(d.status||'')+' · job='+esc(d.job_id||'-')+'</div>';
  }
  if(tasks.length){
    html += '<div class="meta">Plano Alfred ('+tasks.length+' tarefa(s))</div>';
    tasks.forEach(function(t){
      html += '<div class="meta">'+esc((t.id||'')+' '+(t.tool||'')+' ['+(t.status||t.risk||'')+']')+'</div>';
    });
  }
  if(d.requires_confirmation){
    html += '<div class="actions"><button type="button" data-msg="AUTORIZO">Autorizar</button><button type="button" class="ghost" data-msg="CANCELA">Cancelar</button></div>';
  }
  html += '<div class="meta">logs: C:\\\\aura\\\\data\\\\alfred e logs_supervisor\\\\hermes_v10.log</div>';
  return html;
}
async function loadHealth(){
  try{
    ...
    document.getElementById('modelProvider').textContent = (d.ollama_ok?'Ollama':'operador')+' · '+(d.model||'local');
    document.getElementById('modelLatency').textContent = (d.ollama_ms!=null?d.ollama_ms:'--')+'ms';
    const pt = d.paper_trade!==false;
    document.getElementById('badgePaper').textContent = pt?'PAPER':'PAPER OFF';
    const foot=document.getElementById('footFlags');
    if(foot) foot.textContent = 'paper_trade='+(pt?'true':'false')+' · repair='+(d.system_repair_allowed?'on':'off')+' · execution_allowed=false · Ollama :11434';
  }catch(e){document.getElementById('healthStatus').textContent='OFF';}
}
document.addEventListener('click',ev=>{
  const sp=ev.target.closest('.btnSpeak');
  if(sp){
    fetch(API+'/api/speak',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:(lastReply||'').slice(0,400)})});
    return;
  }
...
async function sendMsg(){
  ...
  if(files.length){
    ...
    for(const f of files){
      packed.push({name:f.name, mime:f.type||'', b64:bufToB64(await f.arrayBuffer())});
    }
    const caption=(inp.value||'').trim();
    box.innerHTML += '<div class="msg user">'+esc(caption||('Enviei '+files.map(x=>x.name).join(', ')))+'</div>';
    inp.value=''; if(fileEl) fileEl.value=''; box.scrollTop=box.scrollHeight;
    try{
      const r=await fetch(API+'/api/upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({files:packed,message:caption,session_id:sessionId})});
      const d=await r.json();
      const names=(d.saved||[]).map(s=>s.ok?s.name+' ok':s.name+' falhou').join(', ');
      lastReply=String((d.alfred&&d.alfred.reply)||d.nota||names);
      box.innerHTML += '<div class="msg bot">'+esc(lastReply)+'<span class="meta">inbox · '+esc(names)+'</span>'+renderPlan(d.alfred||{})+'</div>';
    }catch(e){
      box.innerHTML += '<div class="msg bot">Falha no envio: '+esc(e.message)+'</div>';
    }
  ...
  try{
    async function postChat(){
      ...
      try{
        return await fetch(API+'/api/chat',{method:'POST',cache:'no-store',signal:ac.signal,headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,use_memory:true,session_id:sessionId})});
      }finally{clearTimeout(to);}
    }
    let r;
    try{ r=await postChat(); }
    catch(net){ await new Promise(res=>setTimeout(res,700)); r=await postChat(); }
    const d=await r.json();
    ...
    box.innerHTML += '<div class="msg bot">'+esc(d.reply)+'<span class="meta">'+(d.model||'')+' · '+Math.round(d.latency_ms||0)+'ms</span>'+renderPlan(d)+'<div class="actions"><button type="button" class="ghost btnSpeak">Ouvir</button></div></div>';
  }catch(e){
...
let micOn=false;
let micStream=null;
function micBtn(){return document.getElementById('btnMic')}
function micLabel(t,on){
  const b=micBtn(); if(!b) return;
  b.textContent=t;
  b.style.background=on?'#c62828':'#1f6feb';
}
function say(t){ const box=document.getElementById('chatBox'); box.innerHTML += '<div class="msg bot">'+esc(t)+'</div>'; box.scrollTop=box.scrollHeight; }
function encodeWav(samples, sr){
  ...
  let o=44;
  for(let i=0;i<n;i++,o+=2){ let s=Math.max(-1,Math.min(1,samples[i])); v.setInt16(o, s<0?s*0x8000:s*0x7fff, true); }
  const bytes=new Uint8Array(buf); let bin=''; const ch=0x8000;
  for(let i=0;i<bytes.length;i+=ch) bin += String.fromCharCode.apply(null, bytes.subarray(i, i+ch));
  return btoa(bin);
}
...
function downsample(ch, fromSr, toSr){
  if(fromSr===toSr) return ch;
  const ratio=fromSr/toSr, n=Math.floor(ch.length/ratio), out=new Float32Array(n);
  for(let i=0;i<n;i++) out[i]=ch[Math.floor(i*ratio)];
  return out;
}
async function recordWavSeconds(sec){
  const stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,channelCount:1}});
  micStream=stream;
  const mime=(window.MediaRecorder&&MediaRecorder.isTypeSupported('audio/webm;codecs=opus'))?'audio/webm;codecs=opus':(window.MediaRecorder&&MediaRecorder.isTypeSupported('audio/webm')?'audio/webm':'');
  let samples, sr=16000, level=0;
  if(window.MediaRecorder && mime){
    const rec=new MediaRecorder(stream,{mimeType:mime});
    const blobs=[];
    rec.ondataavailable=function(e){ if(e.data&&e.data.size) blobs.push(e.data); };
    rec.start(250);
    await new Promise(function(res){ setTimeout(res, Math.round(sec*1000)); });
    await new Promise(function(res){ rec.onstop=res; try{rec.stop()}catch(e){res()} });
    ...
    samples=downsample(ch, buf.sampleRate, 16000);
    sr=16000;
    level=rmsOf(samples);
    ...
    samples=new Float32Array(len); let o=0;
    chunks.forEach(function(c){ samples.set(c,o); o+=c.length; });
    level=rmsOf(samples);
  }
  return {b64: encodeWav(samples, sr), rms: level, samples: samples.length};
}
async function startMic(){
  const inp=document.getElementById('msgInput');
  if(micOn){ micOn=false; micLabel('Falar',false); if(micStream) micStream.getTracks().forEach(function(t){ t.stop(); }); return; }
  micOn=true; micLabel('A ouvir 8s…',true);
  say('Pode falar. Estou a gravar 8 segundos no microfone do Windows (Realtek), não no browser. Diz «Alfred» no início.');
  try{
    const ac=new AbortController();
    const to=setTimeout(()=>ac.abort(),90000);
    const r=await fetch(API+'/api/listen',{method:'POST',cache:'no-store',signal:ac.signal,headers:{'Content-Type':'application/json'},body:JSON.stringify({seconds:8,session_id:sessionId,prefer:'realtek'})});
    clearTimeout(to);
    const d=await r.json();
    micOn=false; micLabel('Falar',false);
    if(d.loading){ say('O Whisper ainda está a carregar. Espera 10 segundos e clica Falar outra vez.'); return; }
    if(d.error && !d.text){ say('Microfone Windows: '+(d.error||d.nota||'falhou')+'. Confirma que o Realtek não está mudo.'); return; }
    const rms=Number(d.rms||0);
    const dev=d.device||'Windows';
    if(d.silent || (rms>0 && rms<0.008)){
      say('O microfone '+dev+' está em silêncio (nível '+rms.toFixed(4)+'). Definições → Som → Entrada → Conjunto de microfones Realtek, tira o mudo, e Falar outra vez.');
      return;
    }
    let t=String(d.text||d.transcript||'').trim();
    if(!t){ say('Gravei o '+dev+' (nível '+rms.toFixed(4)+') mas não percebi as palavras. Fala mais alto, mais perto, e um pouco mais devagar.'); return; }
    ...
    micOn=false; micLabel('Falar',false);
    say('Falha a gravar no PC: '+String(e.message||e)+'. Recarrega http://127.0.0.1:8777/chat e tenta outra vez.');
  }
}
document.getElementById('btnMic').addEventListener('click',function(ev){ ev.preventDefault(); startMic(); });
const hint=document.getElementById('btnMicHint');
if(hint) hint.addEventListener('click',function(ev){ ev.preventDefault(); startMic(); });
loadHealth();
