...
class HermesSecurityGuard:
    ...
    def check_environment_safety(self) -> bool:
        ...
        flags_path = Path(self.root) / "config" / "aura_flags.json" if hasattr(self, "root") else Path(r"C:\aura\config\aura_flags.json")
        paper_off = False
        try:
            import json
            from pathlib import Path as _P
            fp = _P(r"C:\aura\config\aura_flags.json")
            if fp.exists():
                paper_off = json.loads(fp.read_text(encoding="utf-8")).get("paper_trade") is False
        except Exception:
            paper_off = False
        if paper_off:
            os.environ["PAPER_TRADE"] = "false"
            self._log("WARN", "paper_trade=false por flags autorizadas; execution_allowed continua false")
        else:
