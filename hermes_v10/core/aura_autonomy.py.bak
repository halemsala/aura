...
# -*- coding: utf-8 -*-
"""
Autonomy Engine v2.0 — o maestro (medico + bombeiro + faxineiro).
...
class AutonomyEngine:
    ...
    def _can_fix(self, code: str) -> Tuple[bool, str]:
        if code not in AUTO_FIX_POLICY:
            return False, "fora da politica — so alerta"
        now = time.time()
        ...
        if hour_count >= MAX_FIXES_PER_HOUR:
            return False, "CIRCUIT BREAKER (muitos fixes na ultima hora) — humano necessario"
        return True, ""
    ...
    def _predictive_v2(self) -> None:
        ...
        if isinstance(mem, (int, float)):
            ...
            if mem >= 90:
                self._toast("AURA · RAM CRITICA", f"RAM {mem:.0f}% — agir agora")
                ...
                self._alert("warning", "PRED",
                    f"RAM {mem:.0f}% subindo {slope:.1f}%/ciclo → 90% em ~{eta:.0f} min")
        try:
            ...
            if growth > 0.5 and free_gb < 20:
                ...
                self._alert("warning", "PRED",
                    f"Disco: logs +{growth:.0f}MB/ciclo, {free_gb:.0f}GB livres → cheio em ~{eta_h:.0f}h")
            elif free_gb < 5:
                self._toast("AURA · DISCO CRITICO", f"{free_gb:.1f}GB livres")
    ...
    def _postmortem(self, code: str, reason: str) -> None:
        ...
        timeline = "\n".join(
            f"- {datetime.fromtimestamp(t).strftime('%H:%M:%S')} · {c} · "
        ...
        try:
            fp.write_text(
                f"# POST-MORTEM {code} — {datetime.now():%Y-%m-%d %H:%M}\n\n"
                f"## Motivo\n{reason}\n\n## Estado na hora\n{evid}\n\n"
                f"## Acoes autonomas recentes\n{timeline or '(nenhuma)'}\n\n"
                f"## Proximo passo\n"
                f"- Fix falhou 2x → provavel causa fora do catalogo → triage\n"
                f"- Reproduzir no gym: `python core/aura_gym.py 200`\n",
                encoding="utf-8",
            )
            self._toast(f"AURA · {code}", "Fix falhou — post-mortem salvo")
        except Exception as e:
        ...
        try:
            if self.alerts is not None:
                if hasattr(self.alerts, "send"):
                    self.alerts.send(sev, f"autonomy:{code}", msg, {})
    ...
    def _cycle(self) -> None:
        ...
        if off or not ollama_ok:
            ...
            if rca["action"] == "alert_human":
                self._toast("AURA · OLLAMA OFF", "Causa-raiz detectada — intervencao humana")
            ...
            for root_svc in rca.get("roots") or []:
                ...
                if sucesso:
                    self._alert("info", code,
                        f"OK {code} RESOLVIDO — cascata deve recuperar {rca.get('symptoms')}")
                else:
                    recent_fails = [1 for t, c, s in self.history
                                    if c == code and not s and time.time() - t < 86400]
                    if len(recent_fails) >= 2:
                        self._postmortem(code, "Fix falhou 2x em 24h — breaker implicito")
        ...
        if not code:
            return
        self._alert("warning", code, f"Deteccao autonoma: {code} · {title}")
        if self.maintenance.is_set():
    ...
    def report(self) -> str:
        ...
        for t, code, ok in self.history[-20:]:
            ...
            lines.append(f"  {ts} · {code} · {'OK resolvido' if ok else 'FALHOU (escalado)'}")
