...
def system_status(args, ctx) -> dict:
    ...
    try:
        ...
        if gpu.returncode == 0 and gpu.stdout.strip():
            ...
            info["gpu"] = {"error": (gpu.stderr or "nvidia-smi falhou")[:200]}
    except (FileNotFoundError, subprocess.TimeoutExpired, ValueError) as e:
        info["gpu"] = {"error": str(e)[:200]}
    return info
...
def restart_service(args, ctx) -> dict:
    ...
    from .. import circuit
    gate = circuit.allow(f"restart:{a['service']}")
    if not gate["allowed"]:
        return {"restarted": False, "status": "blocked", "circuit": gate,
                "error": f"circuit breaker aberto — retry em {gate.get('retry_in_s')}s"}
    cfg = get_config()
    if a["service"] == "alfred":
        ...
        circuit.record_success(f"restart:{a['service']}")
...
def _v_tests(args) -> dict:
    return {}


def run_project_tests(args, ctx) -> dict:
    if ctx.dry():
        return {"dry_run": True, "nota": "testes NÃO executados (dry-run)"}
    cmd = [sys.executable, "-m", "pytest", "-q", "--tb=line", "tests/test_alfred_offline.py"]
    try:
        p = subprocess.run(cmd, cwd=str(PROJECT_ROOT), capture_output=True, text=True, timeout=180)
    except subprocess.TimeoutExpired:
        return {"ok": False, "error": "timeout 180s a correr pytest"}
    return {"ok": p.returncode == 0, "returncode": p.returncode,
            "stdout": (p.stdout or "")[-3000], "stderr": (p.stderr or "")[-1500]}


register(ToolSpec("run_project_tests", run_project_tests, _v_tests, risk="low", mutating=False,
                  summary="Corre testes offline do Alfred (pytest, argv fixo, sem shell)"))
