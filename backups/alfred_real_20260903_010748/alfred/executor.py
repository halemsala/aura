...
class Task:
    ...
    timeout: float = 30.0
    retry_policy: dict = field(default_factory=lambda: {"max_retries": 2, "backoff_s": 0.4})
...
def _fmt_tool_result(t: dict) -> str:
    ...
    if tool == "system_status":
        ...
        oll = r.get("ollama") or {}
        return (
            f"CPU {r.get('cpu_percent', '?')}%, RAM {r.get('ram_used_pct', '?')}%. "
            f"GPU {gpu.get('name') or 'n/d'} {gpu.get('vram_used_mib', '?')}/{gpu.get('vram_total_mib', '?')} MiB. "
            f"Ollama {'online' if oll.get('online') else 'offline'}, qwen3:8b "
            f"{'presente' if oll.get('model_present') else 'AUSENTE'}."
        )
    if tool == "aura_stack_status":
        bits = []
        for k in ("alfred", "hermes", "voice", "engine", "matriz", "bridge", "ollama"):
            v = r.get(k) or {}
            bits.append(f"{k}={'ON' if v.get('online') else 'OFF'}")
        return "Stack: " + ", ".join(bits) + "."
    if tool == "services_status":
        ons = [k for k, v in r.items() if isinstance(v, dict) and v.get("online")]
        return "Serviços online: " + (", ".join(ons) if ons else "nenhum") + "."
    if tool == "runtime_flags":
        return (
            f"paper_trade={r.get('paper_trade')} repair={r.get('system_repair_allowed')} "
            f"execution_allowed={r.get('execution_allowed')} modelo={r.get('model')} ctx={r.get('num_ctx')}."
        )
    if tool == "list_tools":
        n = len((r.get("tools") or []))
        return f"Tenho {n} ferramentas activas."
    if r.get("nota"):
        return str(r.get("nota"))[:240]
    return ""
...
def human_summary(job: dict) -> str:
    ...
    if job["status"] == "cancelled":
        ...
        return f"Cancelado a meio: {done} de {len(tasks)} já tinham corrido."
    ...
    narr = " ".join(x for x in (_fmt_tool_result(t) for t in ok) if x).strip()
    if narr:
        base = narr
    elif job.get("plan_intent") == "search_multi" or (tasks and all(t["tool"] == "open_url" for t in tasks)):
        base = f"Abri {len(ok)} pesquisa(s)."
    else:
        names = ", ".join(str(t.get("tool") or "") for t in ok) or "nada"
        base = f"Pronto. {names}."
    if all_dry:
        base += " Ainda é plano: nada alterado. Responde AUTORIZO para executar."
    parts = [base]
    for t in fails:
...
class Executor:
        ...
        for t in job["tasks"]:
            if ctx.cancelled():
                if t["status"] in ("planned", "waiting_confirmation"):
                    ...
                    t["error"] = "cancelado pelo utilizador"
                continue
            deps = t.get("dependency_ids") or []
            if deps:
                failed_deps = [d for d in deps if any(
                    x.get("id") == d and x.get("status") in ("failed", "skipped", "cancelled")
                    for x in job["tasks"])]
                if failed_deps:
                    t["status"] = "skipped"
                    t["error"] = f"dependência falhou: {', '.join(failed_deps)}"
                    continue
            elif failure is not None and job.get("plan_intent") != "search_multi":
                t["status"] = "skipped"
            ...
            self.store.save(entry)
            err = None
            policy = t.get("retry_policy") or {}
            max_retries = int(policy.get("max_retries", cfg.get("max_retries", 2)))
            backoff = float(policy.get("backoff_s", 0.4))
    ...
    def _call(self, spec, t: dict, ctx: Context, cfg: dict) -> dict:
        ...
        timeout = float(t.get("timeout") or cfg.get("task_timeout_s", 30))
