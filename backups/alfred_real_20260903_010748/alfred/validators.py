...
DEFAULT_MUTATE_ROOTS = ["Desktop", "Documents", "Downloads", "Pictures", "Videos", "Music"]


def user_roots(mode: str = "mutate") -> list:
    names = get_config().get("allowed_roots", DEFAULT_MUTATE_ROOTS)
    roots = [home_dir() / n for n in names]
...
SYSTEM_DIR_HINTS = {
    ...
    "system volume information", "system32", "syswow64", "winsxs",
    "system", "$recycle.bin", "recovery", "boot", "efi", "perflogs",
}
...
def resolve_allowed(raw, base_root=None, mode: str = "mutate") -> Path:
    ...
    if p.is_absolute():
        ...
        roots = {r.name.casefold(): r for r in user_roots(mode)}
        if first in roots:
    ...
    system_roots = [
        windir,
        Path(os.environ.get("SYSTEMROOT") or str(windir)),
        Path(r"C:\Windows"),
    ...
    for root in system_roots:
        try:
            rl = str(root.resolve()).casefold()
        except OSError:
            rl = str(root).casefold()
        if cl == rl or cl.startswith(rl + os.sep):
            raise ValidationError("directório de sistema Windows bloqueado")
    ok = False
    for r in user_roots(mode):
        rl = str(r).casefold()
        if cl == rl or cl.startswith(rl + os.sep):
            ok = True
            break
