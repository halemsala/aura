...
<div class="wrap">
  ...
  <div class="chat">
    ...
    <div class="send">
      <textarea id="msgInput" rows="3" placeholder="Alfred, estado  ·  cola código + Alfred, instala esta ferramenta ```python ... ```  ·  Enter envia, Shift+Enter nova linha" onkeydown="if(event.key==='Enter' && !event.shiftKey){event.preventDefault();sendMsg()}"></textarea>
...
async function sendMsg(){
  ...
  try{
    ...
    const r=await fetch(API+'/api/chat',{method:'POST',cache:'no-store',signal:ac.signal,headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,use_memory:true,session_id:sessionId})});
    clearTimeout(to);
    const d=await r.json();
    ...
    box.innerHTML += '<div class="msg bot">'+esc(d.reply)+'<span class="meta">'+(d.model||'')+' · '+Math.round(d.latency_ms||0)+'ms</span>'+renderPlan(d)+'</div>';
  }catch(e){
